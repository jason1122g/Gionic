...
class ReleaseKeyTest extends Specification {
    ...
    def "of( int ) is used for keys like ctrl, shift, alt...etc"(){
        ...
        then:
            1 * simulator.keyRelease( KeyEvent.VK_SHIFT, 1, 0, _ as Character )
    }
    ...
    def "of( char ) is used for Alphabet inputs"(){
        ...
        then:
            1 * simulator.keyRelease( KeyEvent.VK_A, 1, 0, 'a' as Character )
    }
    ...
    def "of( char ) can also handle uppercase"(){
        when:
            release.of( 'A' as Character )
        then:
            1 * simulator.keyRelease( KeyEvent.VK_A, 1, KeyEvent.SHIFT_DOWN_MASK, 'A' as Character )
    }
    ...
    def "forTimes() can specify the repeat times"(){
        when:
            release.forTimes( 3 ).of( '%' as Character )
        then:
            1 * simulator.keyRelease( KeyEvent.VK_5, 3, KeyEvent.SHIFT_DOWN_MASK, '%' as Character )
    }
    ...
    def "ofSomeKey() can use the default key"(){
        when:
            release.ofSomeKey()
        then:
            1 * simulator.keyRelease( _ as Integer, 1, 0, _ as Character )
    }
}
